...
//! This might be useful if you use smol in the parent process but use synchronous code in the
...
//! ## Processes
//!
//! To start a child process, you use the `spawn_smol` method generated by `#[func]`:
//!
//! ```ignore
//! #[func]
//! fn my_process() {
...
//! ```
//!
//! Note that you can use the `spawn_smol` method on both synchronous and asynchronous functions,
...
async fn send_on_fd<T: Object>(fd: &Async<UnixStream>, value: &T) -> Result<()> {
    let mut sender = SingleObjectSender::new(fd.as_raw_fd(), value);
    fd.write_with(|_| sender.send_next()).await
}

async unsafe fn recv_on_fd<T: Object>(fd: &Async<UnixStream>) -> Result<Option<T>> {
    let mut receiver = SingleObjectReceiver::new(fd.as_raw_fd());
    fd.read_with(|_| receiver.recv_next()).await
}

impl<T: Object> Sender<T> {
    unsafe fn from_unix_stream(fd: Async<UnixStream>) -> Self {
        Sender {
            fd,
            marker: PhantomData,
        }
...
/// The subprocess object created by calling `spawn_smol` on a function annottated with `#[func]`.
